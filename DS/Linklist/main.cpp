...
//头插法新建链表
LinkList HeadCreatList(LinkList &L)//list_head_insert
{
	LinkList s;
	int x;
...
//尾插法新建链表
LinkList TailCreatList(LinkList &L)//list_tail_insert
{
	int x;
	L = (LinkList) malloc(sizeof(LNode));//带头节点的链表
	LinkList s, r = L;//LinkList s,r=L;也可以，r代表链表表尾结点，指向链表尾部
	//3 4 5 6 7 9999
	scanf("%d", &x);
	while (x != 9999) {
...
int main() {
	...
	//HeadCreatList(L);//输入数据可以为3 4 5 6 7 9999,头插法新建链表
	TailCreatList(L);//输入数据可以为3 4 5 6 7 9999
	PrintList(L);//链表打印
